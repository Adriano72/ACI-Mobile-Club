...
//var next = _.throttle($.wizard.next, 1000);
var next = function(){
    $.wizard.next();
}
var prev = _.throttle($.wizard.prev, 1000);
...
function initWizard() {
    ...
    var step_iniziale = {
        id: 'step_iniziale',
        view: Alloy.createController('account/confirm_cf/step_iniziale').getView(),
        prev: confirmExit,
        next: 'step_cf'
    };
...
/*
    var form_cf = Alloy.createController('account/confirm_cf/step_cf', {
        onSubmit: function() {
            $.wizard.next();
        }
    });
*/
    ...
    var form_dati_anagrafici = Alloy.createController('account/confirm_cf/step_dati_anagrafici');
    ...
    //step 4: conferma codice
    var form_verify = Alloy.createController('account/confirm_cf/step_verify');
    var step_verify = {
        id: 'step_verify',
        view: form_verify.getView(),
        next: function(done, abort) {
            console.log('step_verify next');
            Alloy.Globals.loading.show('Verifica in corso');
           
            form_verify.validate(function(isValid){
               
                if(isValid){

                    var code = form_verify.read().code;

                    tiACI.Services.SSO.consolidateMobile(code, function(err, res) {
    ...
    $.wizard.addSteps([step_iniziale, step_cf, step_dati_anagrafici, step_cellulare, step_verify]);
}
