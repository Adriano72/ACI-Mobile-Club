...
function mostraMappa() {
	...
	var mapWin = Alloy.createController('mapView', {
		collection : Alloy.Collections.demolitori.toJSON(),
		pin : "pin_Demolitori.png",
		titolo : (OS_ANDROID) ? "Demolitori" : $.titleControl.backgroundImage,
		homeIcon : "ico_demolitori_blu.png"
	}).getView();
	Alloy.Globals.navMenu.openWindow(mapWin);
};

function dettaglioDemolitori(e) {
	var selectedDemolitori = Alloy.Collections.demolitori.getByCid(e.rowData.modelId);

	var dettDemolitori = Alloy.createController('PuntiAci_Demolitori_Dett', {data: selectedDemolitori}).getView();
	Alloy.Globals.navMenu.openWindow(dettDemolitori);
}
