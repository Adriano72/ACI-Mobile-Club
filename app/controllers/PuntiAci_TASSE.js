...
function loadData() {
    ...
    Alloy.Collections.tasse.fetchIfChanged(function(err, cached) {
        updateUI();
...
function dettaglioTasse(e) {
    if (e && e.rowData) {

        var selectedTasse = Alloy.Collections.tasse.getByCid(e.rowData.modelId);

        var dettTasse = Alloy.createController('PuntiAci_Tasse_Dett', {
            data: selectedTasse
        }).getView();
        Alloy.Globals.navMenu.openWindow(dettTasse);
    }
}
