exports.getPuntiAci = function(type_code, _callback) {
	...
	xhr.open('GET', Alloy.Globals.baseURL + '/aci/pos?query={"_type":"' + type_code + '", "status": "ok", "address.location": { "$near": [' + Alloy.Globals.userPosition.longitude + ',' + Alloy.Globals.userPosition.latitude + '], "$maxDistance": 1 } }&limit=15');
...
exports.getDemolitori = function(type_code, _callback) {
	...
	xhr.open('GET', Alloy.Globals.baseURL + '/aci/dem?query={"_type":"' + type_code + '", "status": "ok", "address.location": { "$near": [' + Alloy.Globals.userPosition.longitude + ',' + Alloy.Globals.userPosition.latitude + '], "$maxDistance": 1 } }&limit=15');
...
exports.getVantaggiSoci = function(type_code, _callback) {
	...
	xhr.open('GET', Alloy.Globals.baseURL + '/aci/syc?query={"agreement_id.categories.short_name":"' + type_code + '", "status": "ok", "address.location": { "$near": [' + Alloy.Globals.userPosition.longitude + ',' + Alloy.Globals.userPosition.latitude + '], "$maxDistance": 1 } }&limit=15');
...
exports.getSSOID = function(p_username, p_password, _callback) {
	...
	xhr.open('GET', 'http://col-login.aci.it/index.php?do=login&application_key=mobile&id=login&username=' + p_username + '&password=' + p_password + '');
...
exports.getUserInfo = function(p_ssoid, _callback) {
	...
	xhr.open('GET', 'http://col-login.aci.it/index.php?do=getCustomUserInfo&application_key=mobile&keypass=0my6o9t6&sso-id=' + p_ssoid + '');
